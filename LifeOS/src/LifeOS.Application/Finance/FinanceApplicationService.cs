...
public sealed class FinanceApplicationService : IFinanceApplicationService
{
        ...
        CreateTransferCommand command)
    {
        ...
        var description = string.IsNullOrWhiteSpace(command.Description) ? "Transfer" : command.Description;
        ...
        if (domainResult.IsError)
            return FSharpResult<TransferExecutionResult, DomainError>.NewError(domainResult.ErrorValue);

        var r = domainResult.ResultValue;

        // Persist: transactions, journal entries, updated accounts
        var txs = ListModule.OfSeq(new[] { r.Transfer.WithdrawalTransaction, r.Transfer.DepositTransaction });
        ...
        var result = new TransferExecutionResult(
            WithdrawalTransactionKey: TransactionIdModule.value(r.Transfer.WithdrawalTransaction.Id),
        ...
        UpdateTransactionCommand command)
    {
        ...
        var merchantId = command.MerchantKey is null
            ? tx.MerchantId
            : string.IsNullOrWhiteSpace(command.MerchantKey)
                ? FSharpOption<MerchantId>.None
                : FSharpOption<MerchantId>.Some(MerchantIdModule.fromString(command.MerchantKey));

        var categoryId = command.CategoryKey is null
            ? tx.CategoryId
            : string.IsNullOrWhiteSpace(command.CategoryKey)
                ? FSharpOption<CategoryId>.None
                : FSharpOption<CategoryId>.Some(CategoryIdModule.fromString(command.CategoryKey));

        var description = command.Description ?? tx.Description;

        var memo = command.Memo is null
            ? tx.Memo
            : string.IsNullOrWhiteSpace(command.Memo)
                ? FSharpOption<string>.None
                : FSharpOption<string>.Some(command.Memo);
        ...
        if (command.Amount.HasValue)
        {
            ...
            var amountResult = FinanceServices.TransactionService.updateTransactionAmount(
                accountOpt.Value, updated, newAmount);
        ...
        if (!string.IsNullOrWhiteSpace(command.Status))
        {
            ...
            if (statusParseResult.IsError)
                return FSharpResult<Transaction, DomainError>.NewError(statusParseResult.ErrorValue);
