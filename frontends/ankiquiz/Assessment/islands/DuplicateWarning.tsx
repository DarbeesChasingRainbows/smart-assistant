    ...
    const timer = setTimeout(async () => {
      try {
        const response = await fetch(
          `/api/v1/flashcards/check-duplicates?deckId=${deckId}&question=${encodeURIComponent(question)}`
        );
        if (response.ok) {
        ...
        <div class="text-sm mt-1 space-y-2">
          {result.value.similarCards.slice(0, 3).map((card) => (
            <div key={card.id} class="bg-white/50 rounded p-2">
              <div class="font-medium text-xs text-gray-600">Existing question:</div>
              <div class="text-sm">{card.question.substring(0, 100)}{card.question.length > 100 ? "..." : ""}</div>
            </div>
          ))}
        </div>
      </div>
