# AnkiQuiz Application Launcher (PowerShell version)
...
function Test-Podman {
    if (-not (Get-Command podman -ErrorAction SilentlyContinue)) {
        ...
        Write-Error "Please install Podman first: https://podman.io/getting-started/"
        exit 1
    }
    
    if (-not (Get-Command podman-compose -ErrorAction SilentlyContinue)) {
        Write-Error "Podman Compose is not installed or not in PATH"
        Write-Error "Please install Podman Compose: pip install podman-compose"
        exit 1
    }
}
...
function Stop-Services {
    ...
    try { podman-compose -f podman-compose.yml down 2>$null } catch { }
    try { podman-compose -f podman-compose.yml -f podman-compose.prod.yml down 2>$null } catch { }
}

# Clean up old containers and images
function Invoke-Cleanup {
    Write-Status "Cleaning up old containers and images..."
    try { podman system prune -f 2>$null } catch { }
}
...
function Start-Services {
    ...
    $secretsExist = $false
    try {
        $null = podman secret inspect postgres_password 2>$null
        $secretsExist = $true
    } catch { }
    ...
    if ($Production -and $secretsExist) {
        ...
        Write-Host "  echo 'your_secure_password' | podman secret create postgres_password -"
        Write-Host "  echo 'Host=postgres;Port=5432;Database=ankiquizdb;Username=postgres;Password=your_secure_password' | podman secret create postgres_connection_string -"
        exit 1
    } else {
        if ($secretsExist) {
...
function Wait-ForServices {
    ...
    for ($i = 1; $i -le $maxAttempts; $i++) {
        try {
            $result = podman exec ankiquiz-postgres pg_isready -U postgres 2>$null
...
function Show-Status {
    ...
    Write-Host " podman-compose -f podman-compose.yml logs -f [service-name]"
    Write-Host "To stop:" -ForegroundColor $Colors.Yellow -NoNewline
    Write-Host "     podman-compose -f podman-compose.yml down"
}

# Main execution
function Main {
    Write-Header "AnkiQuiz Application Launcher"
    ...
    switch ($Command.ToLower()) {
        ...
        "logs" {
            if ($Arguments.Count -gt 0) {
                podman-compose -f podman-compose.yml logs -f $Arguments
            } else {
        ...
        "help" {
            Write-Host "AnkiQuiz Launcher Script (PowerShell)"
            Write-Host ""
            ...
            Write-Host ""
            Write-Host "Examples:"
            Write-Host "  .\launch.ps1              # Start in development mode"
            Write-Host "  .\launch.ps1 prod         # Start in production mode"
            Write-Host "  .\launch.ps1 logs backend # Show backend logs"
            Write-Host "  .\launch.ps1 status       # Show service status"
            exit 0
        }
        "start" {
            # Default action
        ...
        default {
            ...
            Write-Host "Use '.\launch.ps1 help' for usage information"
            exit 1
        }
    }
}
