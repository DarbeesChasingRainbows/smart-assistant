  ...
  const lookupVin = async () => {
    ...
    try {
      ...
      const data: VinData = {
        ...
        bodyClass: response.body || "Unknown",
        isRv: response.vehicleType === "rv",
      };

      vinData.value = data;

      if (redirectToAddVehicle && IS_BROWSER) {
        globalThis.sessionStorage?.setItem("vinData", JSON.stringify(data));
        globalThis.location.href = url(redirectTo ?? "/vehicles/add");
      }
    } catch (err) {
      error.value = err instanceof Error ? err.message : "Failed to lookup VIN";
