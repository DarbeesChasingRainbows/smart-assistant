...
export default define.page(async (ctx) => {
  ...
  const uniqueCategories = [
    ...new Set(allDecks.map((d: Deck) => d.category).filter(Boolean)),
  ];
  const uniqueSubcategories = [
    ...new Set(allDecks.map((d: Deck) => d.subcategory).filter(Boolean)),
  ];
  const totalCards = allDecks.reduce(
    (sum: number, d: Deck) => sum + (d.cardCount || 0),
    0,
  );
  ...
  if (difficulty) {
    filteredDecks = filteredDecks.filter((deck: Deck) =>
      deck.difficultyLevel === difficulty
    );
  }
        ...
        <div class="space-y-4">
          <div class="flex items-center justify-between">
            ...
            <div class="flex gap-2">
              <a href={url("/quiz/interleaved")} class="btn btn-primary btn-sm">
                Interleaved Study
              </a>
              <a href={url("/graph")} class="btn btn-ghost btn-sm">
                View Graph
              </a>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
          ...
          <div class="mb-4">
            <form method="GET" action="/decks" class="flex gap-2">
              ...
              {search && (
                <a href={buildUrl({ search: "" })} class="btn btn-ghost">
                  Clear
                </a>
              )}
            </form>
            ...
            <form method="GET" action="/decks" class="contents">
              ...
              >
                ...
                {category && uniqueSubcategories
                  .filter((sub) =>
                    allDecks.some((d: Deck) =>
                      d.category === category && d.subcategory === sub
                    )
                  )
          ...
          <div class="flex flex-wrap gap-2">
            {search && (
              <span class="badge badge-primary gap-2">
                Search: {search}
                <a href={buildUrl({ search: "" })} class="btn btn-ghost btn-xs">
                  ✕
                </a>
              </span>
            )}
        ...
        {filteredDecks.length === 0
          ? (
            <div class="text-center py-12">
              ...
              <a href={url("/decks")} class="btn btn-primary btn-sm mt-4">
                Clear All Filters
              </a>
            </div>
          )
                    ...
                    <div class="flex items-center justify-between pt-2 text-xs text-muted-foreground border-t border-gray-200">
                      <div class="flex items-center gap-3">
                        ...
                        <div class="flex items-center gap-1">
                          ...
                          <span class="badge badge-xs badge-outline">
                            {deck.cardCount?.toString().slice(-1) || "1"}
                          </span>
                        </div>
                      </div>
                    </div>
