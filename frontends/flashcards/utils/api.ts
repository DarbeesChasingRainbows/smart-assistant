...
export class RetentionApiClient {
  ...
  async generateQuiz(
    ...
    const response = await fetch(
      `${this.baseUrl}/v1/flashcards/quiz?${params.toString()}`,
      {
        method: "POST",
      },
    );
  ...
  async submitQuizResult(quizResult: QuizResultRequest): Promise<void> {
    const response = await fetch(`${this.baseUrl}/v1/flashcards/quiz-result`, {
      method: "POST",
  ...
  async reviewFlashcard(reviewRequest: ReviewRequest): Promise<void> {
    const response = await fetch(`${this.baseUrl}/v1/flashcards/review`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  async getFlashcards(deckId?: string | null): Promise<Flashcard[]> {
    let url = `${this.baseUrl}/v1/flashcards`;
  ...
  async getFlashcardById(id: string): Promise<Flashcard> {
    const response = await fetch(`${this.baseUrl}/v1/flashcards/${id}`);
    if (!response.ok) {
  ...
  async createFlashcard(request: CreateFlashcardRequest): Promise<Flashcard> {
    const response = await fetch(`${this.baseUrl}/v1/flashcards`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  async updateFlashcard(flashcard: Flashcard): Promise<Flashcard> {
    const response = await fetch(
      `${this.baseUrl}/v1/flashcards/${flashcard.id}`,
      {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  async getDecks(): Promise<Deck[]> {
    const response = await fetch(`${this.baseUrl}/v1/decks`);
    if (!response.ok) {
  ...
  async getDeckById(id: string): Promise<Deck> {
    const response = await fetch(`${this.baseUrl}/v1/decks/${id}`);
    if (!response.ok) {
  ...
  async getGlossaryTerms(deckId: string): Promise<GlossaryTerm[]> {
    const response = await fetch(`${this.baseUrl}/v1/decks/${deckId}/glossary`);
    if (!response.ok) {
  ...
  async getCrossReferences(deckId: string): Promise<CrossReference[]> {
    const response = await fetch(
      `${this.baseUrl}/v1/decks/${deckId}/cross-references`,
    );
    if (!response.ok) {
      throw new Error(
        `Failed to fetch cross references: ${response.statusText}`,
      );
    }
    return response.json();
  }
  ...
  async getDeckFlashcards(deckId: string): Promise<Flashcard[]> {
    const response = await fetch(
      `${this.baseUrl}/v1/decks/${deckId}/flashcards`,
    );
    if (!response.ok) {
      throw new Error(
        `Failed to fetch deck flashcards: ${response.statusText}`,
      );
    }
    return response.json();
  }
  ...
  async getDeckFlashcardVersions(deckId: string): Promise<Flashcard[]> {
    const response = await fetch(
      `${this.baseUrl}/v1/decks/${deckId}/flashcard-versions`,
    );
    if (!response.ok) {
      throw new Error(
        `Failed to fetch deck flashcard versions: ${response.statusText}`,
      );
    }
    return response.json();
  }
  ...
  async getDeckStats(
    deckId: string,
  ): Promise<{ cardCount: number; averageDifficulty: number }> {
    const response = await fetch(`${this.baseUrl}/v1/decks/${deckId}/stats`);
    if (!response.ok) {
  ...
  async importDeck(deckName: string, file: File): Promise<DeckImportResult> {
    ...
    const response = await fetch(
      `${this.baseUrl}/v1/decks/import?deckName=${
    ...
    if (!response.ok) {
      throw new Error(`Failed to import deck: ${response.statusText}`);
    }
    return response.json();
  }

  async importDirectory(directoryPath: string): Promise<ImportResult> {
    const response = await fetch(`${this.baseUrl}/v1/decks/import-directory`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  async identifyUser(request: IdentifyUserRequest): Promise<User> {
    const response = await fetch(`${this.baseUrl}/v1/users/identify`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(request),
    });
  ...
  async recordQuizCompletion(
    ...
    const response = await fetch(
      `${this.baseUrl}/v1/users/${userId}/quiz-completed`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(request),
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to record quiz completion: ${response.statusText}`,
      );
    }
    return response.json();
  }
  ...
  async recordCardReview(
    ...
    const response = await fetch(
      `${this.baseUrl}/v1/users/${userId}/card-reviewed`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(request),
    ...
    if (!response.ok) {
      throw new Error(`Failed to record card review: ${response.statusText}`);
    }
    return response.json();
  }

  async getLeaderboard(
    type: "streak" | "quizzes" | "accuracy",
  ): Promise<User[]> {
    const response = await fetch(
      `${this.baseUrl}/v1/users/leaderboard/${type}`,
    );
    if (!response.ok) {
